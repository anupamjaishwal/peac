...
import fetchRecords from '@salesforce/apex/pEAC_ReusableLookupController.fetchRecords';
const DELAY = 500;
export default class PEAC_ReusableLookupLWC extends LightningElement {
    ...
    @api filterCriteriaList = []; // Provide in this format [{"fieldAPIName":"", "operator":"","value":"","datatype":""}];
    ...
    fetchSobjectRecords(loadEvent) {
        console.log('ReusableLookup::',this.methodInput);
        fetchRecords({
            inputWrapper: this.methodInput
        }).then(result => {
        console.log('ReusableLookup::',result);
            if (loadEvent && result) {
