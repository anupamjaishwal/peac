...
export default class Sl_datatablePaginator extends LightningElement {
  ...
  //! This loads the size options, hides/unhides the pagination depending on the number of records and more...
  connectedCallback() {
  ...
  //? Handles the record per page button click
  ...
  @api setRecordsToDisplay() {
    ...
    this.totalPages =
      Math.ceil(this.totalRecords / this.pageSize) === 0
        ? 1
    ...
    for (
      ...
      if(this.records[i]) this.recordsToDisplay.push(this.records[i]);
    }
    this.dispatchEvent(
